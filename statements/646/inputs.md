# Erdős Problem 646 — drafting inputs
...
<title>646 | Erdős Problems</title>
<meta property="og:title" content="646 | Erdős Problems">
...
<div class="container">
  ...
  The answer is yes, proved by Berend \cite{Be97}, who further proved that the sequence of such $n$ has bounded gaps (where the bound depends on the initial set of primes).
  ...
  [Be97] Berend, Daniel, <i>On the parity of exponents in the factorization of {$n!$}</i>. J. Number Theory (1997), 13-19.
  
  </div>
                                        
  <div class="problem-additional-text" style="text-align:center; padding: 10px;">
  <a href="/646">Back to the problem</a>
  </div>
...
## Hosted theorem — plby (https://github.com/plby/lean-proofs/blob/main/src/v4.29.1/ErdosProblems/Erdos646.lean)

```lean
theorem infinitely_many_even_factorial_exponents (k : ℕ) (p : Fin k → ℕ) (hp : ∀ i, (p i).Prime) (h_distinct : Function.Injective p) :
  Set.Infinite { n | ∀ i, partial_sum k p n i = 0 }
```

## Hosted theorem — jayyhk (https://github.com/Jayyhk/erdos-lean/blob/main/problems/646/Erdos646.lean)

```lean
theorem erdos_646 (S : Finset ℕ) (hS : ∀ p ∈ S, Nat.Prime p) :
    Set.Infinite { n : ℕ | ∀ p ∈ S, Even (padicValNat p (Nat.factorial n)) }
```

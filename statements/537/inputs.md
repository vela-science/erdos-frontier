# Erdős Problem 537 — drafting inputs
...
<title>537 | Erdős Problems</title>
<meta property="og:title" content="537 | Erdős Problems">
...
## Hosted theorem — plby (https://github.com/plby/lean-proofs/blob/main/src/v4.29.1/ErdosProblems/Erdos537.lean)
...
theorem main_theorem (hChebyshev : ChebyshevUpperBound) :
  HasNaturalDensity SpecialSet ∧ naturalDensity SpecialSet > 0
```
...
## Hosted theorem — jayyhk (https://github.com/Jayyhk/erdos-lean/blob/main/problems/537/Erdos537.lean)

```lean
theorem answer (h_density : ∃ c > 0, ∃ N₀, ∀ N ≥ N₀, ((SpecialFinset N).card : ℝ) ≥ c * N) : ¬ RothLikeProperty
```
...
```lean
theorem erdos_537 : ¬(∀ (ε : ℝ), 0 < ε → ∃ (N₀ : ℕ), ∀ (N : ℕ), N₀ ≤ N →
  ∀ (A : Finset ℕ), A ⊆ Finset.Icc 1 N → (A.card : ℝ) ≥ ε * N →
  ∃ a₁ ∈ A, ∃ a₂ ∈ A, ∃ a₃ ∈ A, ∃ p₁ p₂ p₃, p₁.Prime ∧ p₂.Prime ∧ p₃.Prime ∧
  p₁ ≠ p₂ ∧ p₁ ≠ p₃ ∧ p₂ ≠ p₃ ∧ a₁ * p₁ = a₂ * p₂ ∧ a₂ * p₂ = a₃ * p₃)
```

# Erdős Problem 484 — drafting inputs
...
<title>484 | Erdős Problems</title>
<meta property="og:title" content="484 | Erdős Problems">
...
<div class="container">
  ...
  A conjecture of Roth.<br><br>Solved by Erd\H{o}s, S\'{a}rk\"{o}zy, and S\'{o}s \cite{ESS89}, who in fact prove that there are at least\[\frac{N}{2}-O(N^{1-1/2^{k+1}})\]many even numbers which are of this form. They also prove that if $k=2$ then there are at least\[\frac{N}{2}-O(\log N)\]many even numbers which are of this form, and that $O(\log N)$ is best possible, since there is a $2$-colouring such that no power of $2$ is representable as a monochromatic sum.<br><br>A refinement of this problem appears as Problem 25 on the <a href="https://people.maths.ox.ac.uk/greenbj/papers/open-problems.pdf" rel="nofollow noopener noreferrer ugc" target="_blank">open problems list</a> of Ben Green.
  ...
  [ESS89] Erd\H{o}s, P. and S\'{a}rk\"ozy, A. and S\'{o}s, V. T., <i>On a conjecture of {R}oth and some related problems. I</i>.  (1989), 47--59.
  
  </div>
                                        
  <div class="problem-additional-text" style="text-align:center; padding: 10px;">
  <a href="/484">Back to the problem</a>
  </div>
...
## Hosted theorem — plby (https://github.com/plby/lean-proofs/blob/main/src/v4.29.1/ErdosProblems/Erdos484.lean)
...
theorem monochromatic_sums_linear :
    ∃ c : ℝ, c > 0 ∧
      ∀ k : ℕ, k ≥ 1 →
        ∃ N₀ : ℕ, ∀ N : ℕ, N ≥ N₀ →
          ∀ f : ℕ → Fin k,
            (monochromaticSumSet N k f).card ≥ ⌊c * (N : ℝ)⌋₊
```

## Hosted theorem — jayyhk (https://github.com/Jayyhk/erdos-lean/blob/main/problems/484/Erdos484.lean)

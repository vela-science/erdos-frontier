# Erdős Problem 443 — drafting inputs
...
<title>443 | Erdős Problems</title>
<meta property="og:title" content="443 | Erdős Problems">
...
<div class="container">
  ...
  [He25] N. Hegyv\'{a}ri, <i>An elementary question of Erd\H{o}s and Graham</i>. arXiv:2503.24201 (2025).
  
  </div>
                                        
  <div class="problem-additional-text" style="text-align:center; padding: 10px;">
  <a href="/443">Back to the problem</a>
  </div>
...
## Hosted theorem — plby (https://github.com/plby/lean-proofs/blob/main/src/v4.29.1/ErdosProblems/Erdos443.lean)

```lean
theorem erdos_443_part_one (s : ℕ) :
  ∃ m n : ℕ, n < m ∧ s ≤ ((A n ∩ A m).card : ℝ)
```

```lean
theorem erdos_443_part_two (ε : ℝ) (hε : 0 < ε) :
  ∃ n₀ : ℕ, ∀ m n : ℕ, n₀ < n → n < m →
  ((A n ∩ A m).card : ℝ) < ((m : ℝ) * n) ^ (ε)
```

## Hosted theorem — jayyhk (https://github.com/Jayyhk/erdos-lean/blob/main/problems/443/Erdos443.lean)
...
```lean
/-- **Erdős Problem 443.** The intersection `{k(m-k) : 1≤k≤m/2} ∩ {l(n-l) : 1≤l≤n/2}`
(here `A n ∩ A m`) can be made arbitrarily large, and for every `ε > 0` it is below
`(m·n)^ε` once `m > n` are large enough — i.e. it is `(mn)^{o(1)}`. -/
theorem erdos_443 :
    (∀ s : ℕ, ∃ m n : ℕ, n < m ∧ s ≤ ((A n ∩ A m).card : ℝ)) ∧
